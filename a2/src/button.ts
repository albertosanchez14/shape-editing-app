...
export class SKButtonCustom extends SKElement {
  ...
  handleMouseEvent(me: SKMouseEvent) {
    ...
    switch (me.type) {
      case "mousedown":
        this.enabled ? this.state = "down" : this.state = "idle";
        requestMouseFocus(this);
        return true;
        break;
      case "mouseup":
        this.enabled ? this.state = "hover": this.state = "idle";
  ...
  draw(gc: CanvasRenderingContext2D) {
    ...
    // set ennabled state
    this.enabled ? (gc.globalAlpha = 1) : (gc.globalAlpha = 0.5);
